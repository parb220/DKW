function [logL,logL_vec,state,model_IE_options]= logL_DKWv_option(pvar,paras_vec,pidx,Nfac,Dt, ...              
...
for j=1:T     
    ...
    if notmissing_options(j) == 1
        b = [b;[zeros(size(bI_Q,1),1),bI_Q,cI_Q]];  
        a = [a;aI_Q];
...
%         yt_tm1(end-Ncaps+1:end)=tmp_prc_caps;
%     end
