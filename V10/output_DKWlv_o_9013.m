...
load '.\results\pvar_DKWlv_o_9012.mat';
data_FRBA; 
[Y,M,D]=datevec(mydate);
...
paras_vec=[reshape(KAPPA,Nfac^2,1);reshape(SIGMA,Nfac^2,1);theta;
      ...
      KAPPA_L;SIGMA_L;theta_L;rho1_L;rhoL_L;lambda0_L;SIGMAlambda1_L];
...
[logL,logL_vec,state,model_IE_options]= logL_DKWlv_o(pvar,paras_vec,pidx,Nfac,Dt, ...              
...
pvar_mat(7:11,:)=[rho0;rho1;rhov]; % [rho0;rho1;rhov];
pvar_mat(12:23,:)=[lambda0;SIGMAlambda1(:)]; % [lambda0;SIGMAlambda1(:)];
pvar_mat(24:28,:)=[rho0_pi; rho1_pi; rhov_pi]; % [rho0_pi; rho1_pi;rhov_pi];
...
pvar_mat(60:61,:)=100*[delta_options]; % [delta_options];

matrix2latex(pvar_mat, '.\results\pvar_DKWlv_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
...
for i=1:length(horLIST);
    ...
    [Ay_TR,By_TR,Cy_TR]=YieldFacLoad_ODE3(MAT,KAPPAtheta_rn_R,KAPPA_rn_R,KAPPAv*thetav,SIGMA*SIGMA',rho0_R,rho1_R+rho1_L,rhov_R,KAPPAv_rn,SIGMAv^2); 
    ay_TR1=-Ay_TR./MAT; ay_TR1=ay_TR1';
    by_TR1=-By_TR./repmat(MAT,Nfac,1); by_TR1=by_TR1';
    cy_TR1=-Cy_TR./MAT; cy_TR1=cy_TR1';
    ...
    ay_L1=-Ay_L./MAT; ay_L1=ay_L1';
    by_L1=-By_L./MAT; by_L1=by_L1';
    
    LRP(:,i)=(ay_TR1+state(:,2:4)*by_TR1'+state(:,5)*cy_TR1)+(ay_L1+state(:,6)*by_L1)-(ay_R1+state(:,2:4)*by_R1'+state(:,5)*cy_R1);
    
    if MAT==10
        BEI10_act_tips=y_vec(:,end)-ytips_vec(:,end);
        BEI10_model_tips=(ay1+state(:,2:4)*by1'+state(:,5)*cy1)-((ay_TR1+state(:,2:4)*by_TR1'+state(:,5)*cy_TR1)+(ay_L1+state(:,6)*by_L1'));
        BEI10_model_true=(ay1+state(:,2:4)*by1'+state(:,5)*cy1)-(ay_R1+state(:,2:4)*by_R1'+state(:,5)*cy_R1);
    end
...
title('Inflation Expectations implied by 1-year inf. options under forward measure');
subplot(2,1,2); plot([model_IE_options(id_options,2),IE_options_avg(id_options,2)]); 
title('Inflation Expectations implied by 3-year inf. options under forward measure');
...
matrix2latex([MAT_list,HOR_list,yN_is_RW_rmse(:)*100,yN_is_rmse(:)*100], '.\results\isyN_DKWlv_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
...
%                            +cy_R1*(thetav+exp(-KAPPAv*hor*Dt)*(state(i-hor,5)-thetav));
            yRhat_is(i)=ay_TR1+by_TR1*(theta+expm(-KAPPA*hor*Dt)*(state(i-hor,2:4)'-theta)) ...
                           +cy_TR1*(thetav+exp(-KAPPAv*hor*Dt)*(state(i-hor,5)-thetav)) ...
                       +ay_L1+by_L1*(theta_L+exp(-KAPPA_L*hor*Dt)*(state(i-hor,6)-theta_L));
...
matrix2latex([MAT_list,HOR_list,yR_is_RW_rmse(:)*100,yR_is_rmse(:)*100], '.\results\isyR_DKWlv_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
...
matrix2latex([MAT_list,HOR_list,yN_oos_RW_rmse(:)*100,yN_oos_rmse(:)*100], '.\results\oosyN_DKWlv_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
...
%                               +cy_R1*(thetav+exp(-KAPPAv*hor*Dt)*(state_oos(i-hor,5)-thetav));
            yRhat_oos(i)=ay_TR1+by_TR1*(theta+expm(-KAPPA*hor*Dt)*(state_oos(i-hor,2:4)'-theta)) ...
                              +cy_TR1*(thetav+exp(-KAPPAv*hor*Dt)*(state_oos(i-hor,5)-thetav)) ...
                        +ay_L1+by_L1*(theta_L+exp(-KAPPA_L*hor*Dt)*(state_oos(i-hor,6)-theta_L));
...
matrix2latex([MAT_list,HOR_list,yR_oos_RW_rmse(:)*100,yR_oos_rmse(:)*100], '.\results\oosyR_DKWlv_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
