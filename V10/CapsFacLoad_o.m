function [ay_caps,by_caps,model_prc_caps] = CapsFacLoad_o(xt_tm1,KAPPA,SIGMA,theta,rho0,rho1, ...
...
%     bx_Q=(eye(Nfac)-expm(-KAPPA_Q'*tau))*inv(KAPPA_Q'*tau);    
    ...
    for i_k=1:Nk
        ...
        id=i_k+(i_tau-1)*Nk;
        ay_caps(id)=-tau*ay(i_tau)+tmp_a-tmp_b'*xt_tm1;
        by_caps(id,:)=-tau*by(i_tau,:)+tmp_b';
        
        model_prc_caps(id)=-tau*(ay(i_tau)+by(i_tau,:)*xt_tm1)+tmp_a;
    end
end
