...
load '.\results\pvar_FRBA_o_9012.mat';
data_FRBA; 
[Y,M,D]=datevec(mydate);
...
delta_options=pall(3*Nfac^2+5*Nfac+8+Ny+Nytips);

lambda1=inv(SIGMA)*SIGMAlambda1;

[logL,logL_vec,state]= logL_FRBA_o(pvar,paras_vec,pidx,Nfac,Dt, ...              
...
pvar_mat(51:51,:)=100*[pvar(43)]; % [delta_options];

matrix2latex(pvar_mat, '.\results\pvar_FRBA_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
...
matrix2latex([MAT_list,HOR_list,yN_is_RW_rmse(:)*100,yN_is_rmse(:)*100], '.\results\isyN_FRBA_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
...
matrix2latex([MAT_list,HOR_list,yR_is_RW_rmse(:)*100,yR_is_rmse(:)*100], '.\results\isyR_FRBA_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
...
for i=1:length(MATgrid_options)
    for j=1:length(STRIKEgrid_options)
        ...
        [ay,by]=YieldFacLoad(KAPPA,SIGMA,theta,rho0,rho1,lambda0,SIGMAlambda1,MAT);
        tmp_Ay=-MAT*ay; tmp_By=-MAT*by;
        
        
        KAPPA_Q=KAPPA+SIGMAlambda1;
        theta_Q=inv(KAPPA_Q)*(KAPPA*theta-SIGMA*lambda0-SIGMA*SIGMA'*by'*MAT);

        rho0_Q=rho0_pi-sigq'*lambda0-sigq'*SIGMA'*by'*MAT;
        rho1_Q=rho1_pi-lambda1'*sigq;

        ax_Q=(eye(Nfac)-inv(KAPPA_Q*MAT)*(eye(Nfac)-expm(-KAPPA_Q*MAT)))*theta;
        bx_Q=(eye(Nfac)-expm(-KAPPA_Q'*MAT))*inv(KAPPA_Q'*MAT);

        W_Q=inv(KAPPA_Q)*(eye(Nfac)-expm(-KAPPA_Q*MAT))/MAT;
        ...
        H0=((sigq+SIGMA'*inv(KAPPA_Q')*rho1_Q)'*(sigq+SIGMA'*inv(KAPPA_Q')*rho1_Q)+sigqx^2)*MAT ...
        ...
        model_caps_vec(:,tmpcount)= exp(tmp_Ay+state(:,2:4)*tmp_By').* ...
               (exp(MAT*(tmpIE+1/2*tmpIU)).*normcdf(tmph1)-(1+STRIKE)^MAT*normcdf(tmph2));
...
matrix2latex([MAT_list,HOR_list,yN_oos_RW_rmse(:)*100,yN_oos_rmse(:)*100], '.\results\oosyN_FRBA_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
...
matrix2latex([MAT_list,HOR_list,yR_oos_RW_rmse(:)*100,yR_oos_rmse(:)*100], '.\results\oosyR_FRBA_o_9012.tex', 'rowLabels', rowLabels, 'columnLabels', columnLabels, 'alignment', 'c', 'format', '%-6.4f', 'size', 'tiny');
