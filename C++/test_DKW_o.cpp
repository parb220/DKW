...
int main(int argc, char **argv)
{
	...
	double Dt = 7.0/365.0; 
	CData_FRBA data(Dt); 
	CDate start_date(1990,1,1), end_date(2015,12,31); 
	...
	int Nfac = 3; 
	CAR_DKW_o model(Nfac, &data); 
	model.SetAsFixed(DiagonalMatrix(MINUS_INFINITY, Nfac, Nfac), string("KAPPA")); 
	...
	model.SetAsFixed(TDenseVector(Nfac,0.0), string("theta")); 
