...
#include "CAR_DKWos.hpp"
#include "CData_FRBA.hpp"

CAR_DKWos :: CAR_DKWos(int _Nfac, CData_FRBA *_dataP) :
CAR_DKWl_s(_Nfac, _dataP), 
...
CAR_DKWos :: CAR_DKWos(const CAR_DKWos &right) :
CAR_DKWl_s(right),
...
TDenseVector CAR_DKWos::GetParameters()
{
	if (!if_internal_parameter_set)
	{
		TDenseVector complete_parameter(NumberParameters(),0.0); 
		complete_parameter.Insert(0, CAR_DKWl_s::GetParameters()); 
		int counter = CAR_DKWl_s::NumberParameters(); 
...
bool CAR_DKWos::SetParameters_FromVectorToMatrix(const TDenseVector &_parameter)
{
	if (_parameter.Dimension() < NumberParameters())
		return false; 

	if (!CAR_DKWl_s::SetParameters_FromVectorToMatrix(_parameter.SubVector(0,CAR_DKWl_s::NumberParameters()-1))) 
		return false; 
...
bool CAR_DKWos::SetParameters_InitializeABOmega()
{
        if (!CAR_DKWl_s::SetParameters_InitializeABOmega())
                return false;
...
TDenseVector CAR_DKWos::ObservationEquation(int j, const TDenseVector &xt_tm1)
{
	TDenseVector model_IE_options_vector = CAR_DKWl_s::ObservationEquation(j, xt_tm1);
...
bool CAR_DKWos:: SetAsFixed(const TDenseVector &v, const std::string &which_one)
{
	if (iequals(which_one, std::string("delta_options")) )
        {
                int offset = CAR_DKWl_s::NumberParameters();
                TIndex local_fixed_index = FixedVariableParameter(delta_options, v, offset);
                FixedIndex.UniqMerge(local_fixed_index);
                if_variable_index_set = false;
                return true;
        }
        else
                return CAR_DKWl_s::SetAsFixed(v, which_one);

}

int CAR_DKWos::NumberParameters() const
{
        return CAR_DKWl_s::NumberParameters() + dataP->MATgrid_options.Dimension();
}
